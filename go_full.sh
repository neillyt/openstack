...
echo "Suggestions: ";
echo ""
echo "Do not run this script remotely! It will restart the network. If your IP settings";
echo "are wrong, you will not be able to log back in.";
echo "";
echo "Run this script as the root user.";
echo "Proceed? Ok.";
echo "";

echo "Which interface are we configuring? Hint: Use 'ip addr'";
read interface;
echo "";

echo "What should our IP be?";
read ip;
echo "";
...
echo "IPADDR=$ip" >> ifcfg-$interface;
echo "NETMASK=255.255.255.224" >> ifcfg-$interface;
echo "GATEWAY=136.142.139.129" >> ifcfg-$interface;
echo "DNS1=136.142.57.10" >> ifcfg-$interface;
cd ~;
